...
// архитектуры с big endian нуждаются в #define __BYTE_ORDER __BIG_ENDIAN
#ifndef _MSC_VER
#include <endian.h>
#endif

// то же самое, что и ресет()
SHA256::SHA256()
{
...
namespace
{
    ...
    // смешанные функции для processBlock()
...
void SHA256::processBlock(const void* data)
{
    ...
    // данные представлены в виде 16 x 32-битных слов
    const uint32_t* input = (uint32_t*)data;
    // преобразовать в big endian
    uint32_t words[64]{};
...
void SHA256::add(const void* data, size_t numBytes)
{
    ...
    // хранить оставшиеся байты в буфере
    while (numBytes > 0)
    {
        if (m_bufferSize < BlockSize) {
            m_buffer[m_bufferSize++] = *current++;
...
// обработать последний блок, менее 64 байт
void SHA256::processBuffer()
{
    // входные байты рассматриваются как битовые строки, где первый бит - старший бит байта

    // - добавляем в сообщение бит "1"
    // - добавлять "0" биты до тех пор, пока длина сообщения в bit mod 512 не будет равна 448
    // - добавляем длину как 64-битное целое число

    // количество битов
    size_t paddedLength = m_bufferSize * 8;

    // плюс один бит, равный 1 (всегда добавляется)
    paddedLength++;

    // количество бит должно быть (numBits % 512) = 448
    ...
    // требуется только в том случае, если дополнительные данные перетекают во второй блок
    ...
    // добавить бит "1", 128 => двоичный код 10000
    if (m_bufferSize < BlockSize)
    ...
    // добавить длину сообщения в битах как 64-битное число
...
{
    // вычислить хэш (в виде необработанных байтов)
...
{
    // сохранять старый хэш, если буфер частично заполнен
