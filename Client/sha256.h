...
{
public:
    // разбивает на блоки по 64 байта (=> 512 бит), длина хэша составляет 32 байта
    enum { BlockSize = 512 / 8, HashBytes = 32 };

    // то же самое, что и сброс ()
    SHA256();

    // вычисляет SHA256 блока памяти
    std::string operator()(const void* data, size_t numBytes);
    // вычислияет SHA256 строки, исключая последний ноль
    std::string operator()(const std::string& text);

    // добавляет произвольное количество байтов
...
private:
    ...
    //обрабатывает все, что осталось во внутреннем буфере
    void processBuffer();
